...
export const patientService = {
  ...
  addAssessment: async (
    ...
    try {
      const res = await api.post("/assessments", {
        ...
        recommendations:
          assessmentData.shortTermGoals || assessmentData.longTermGoals || "",
      });
      return res.data;
    } catch {
      console.warn(
  ...
  updatePlanStatus: async (
    ...
    try {
      const res = await api.patch(`/treatment-plans/${planId}/status`, {
        status,
      });
      return res.data;
    } catch {
  ...
  createPrescription: async (
    ...
    try {
      const res = await api.post("/exercise-prescriptions", {
        patientId: parseInt(patientId, 10) || patientId,
        instructions:
          prescriptionData.generalInstructions || prescriptionData.targetGoal,
        status: "Active",
      });
      return res.data;
    } catch {

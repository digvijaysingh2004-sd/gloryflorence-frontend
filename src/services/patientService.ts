    ...
    medicalHistory: [
      {
        ...
        status: "Active",
        notes:
          "Pain increases after long sitting hours. Lumbar L4-L5 disc compression.",
      },
      {
        id: "mh_2",
    ...
    assessments: [
      {
        ...
        chiefComplaint:
          "Sharp lumbar spine pain radiating down right buttock and posterior thigh after prolonged desk sitting.",
        painScore: 7,
        painLocation: "Lumbar Spine (L4-L5) & Right Gluteal",
        painType: "Radiating",
        aggravatingFactors:
          "Sitting >30 mins, forward spinal flexion, coughing",
        relievingFactors:
          "Walking, lying prone with lumbar pillow, heat application",
        romFindings:
          "Lumbar flexion restricted to 45° with peripheralization. Extension limited to 10° due to sharp pain. Positive SLR on right at 40°.",
        postureAndGait:
          "Mild antalgic gait favoring right side. Flattened lumbar lordosis.",
        functionalLimitations:
          "Unable to sit continuously for work meetings. Difficulty lifting objects >5kg.",
        clinicalDiagnosis:
          "L4-L5 Disc Herniation with Right Lumbar Radiculopathy",
        prognosis: "Good",
        shortTermGoals:
          "Reduce VAS pain score from 7/10 to 4/10 in 3 weeks. Centralize radicular symptoms to lumbar spine.",
        longTermGoals:
          "Full pain-free lumbar ROM, return to recreational swimming and desk work 2+ hours without discomfort.",
        recommendedFrequency: "3 sessions / week for 4 weeks",
        notes:
          "Advised ergonomic workstation changes and avoidance of early morning heavy spinal loading.",
      },
    ],
    treatmentPlans: [
      {
        ...
        status: "Active",
        goals:
          "Reduce pain level from 7/10 to 2/10. Increase core strength. Return to swimming.",
        treatments: [
        ...
        sessions: [
          {
            ...
            date: "2026-08-28",
            notes:
              "Completed initial evaluation and first decompression mobilization. Patient reported 3 points pain relief immediately following traction.",
            ...
            nextSessionPlan:
              "Reassess SLR angle and initiate transverse abdominis isometric activation.",
          },
        ],
      },
    ],
    prescriptions: [
      {
        ...
        status: "Active",
        targetGoal:
          "Restore spinal extension tolerance and strengthen multifidus and transverse abdominis to stabilize lumbar spine.",
        generalInstructions:
          "Perform movements smoothly without breath-holding. If sharp pain or numbness radiates below the knee, discontinue that movement and notify your clinician.",
        items: [
          {
            id: "rxi_1",
            exerciseId: "ex_8",
            ...
            durationWeeks: 4,
            notes:
              "Exhale completely at the top of the press-up. Keep hips relaxed on the mat.",
          },
          {
            id: "rxi_2",
            exerciseId: "ex_3",
            exerciseTitle:
              "Bird-Dog Core Stability (Quadruped Cross-Extension)",
            ...
            durationWeeks: 4,
            notes:
              "Maintain neutral spine; avoid hyperextending or rotating the pelvis.",
          },
          {
            id: "rxi_3",
            exerciseId: "ex_1",
            ...
            durationWeeks: 3,
            notes:
              "Squeeze glutes at top position. Ensure hamstrings do not cramp.",
          },
        ],
      },
    ],
    ...
    medicalHistory: [
      {
        ...
        status: "Chronic",
        notes:
          "Impingement signs positive. Restricted abduction above 90 degrees.",
      },
    ],
    documents: [
      {
    ...
    assessments: [
      {
        ...
        chiefComplaint:
          "Inability to lift right arm overhead with acute impingement and night ache.",
        painScore: 8,
        painLocation: "Right Shoulder Anterior & Lateral Deltoid",
        painType: "Dull Aching",
        aggravatingFactors:
          "Overhead reaching, sleeping on right side, putting on jacket",
        relievingFactors: "Ice pack, arm rest in sling",
        romFindings:
          "Active abduction limited to 85° with painful arc between 60°-120°. Hawkins-Kennedy test positive.",
        postureAndGait:
          "Protracted right shoulder girdle with slight anterior head carriage.",
        functionalLimitations:
          "Cannot comb hair or reach upper kitchen shelves.",
        clinicalDiagnosis:
          "Supraspinatus Tendinopathy with Subacromial Impingement",
        prognosis: "Good",
        shortTermGoals:
          "Abolish night pain, increase abduction to 120° in 4 weeks.",
        longTermGoals:
          "Complete pain-free overhead active ROM and return to resistance training.",
    ...
    treatmentPlans: [
      {
        ...
        status: "Completed",
        goals:
          "Restore shoulder abduction to 170 degrees. Pain-free overhead reaching.",
        treatments: [
          "Manual therapy & joint mobilization",
        ...
        sessions: [
          {
            ...
            date: "2025-05-12",
            notes:
              "Final rehabilitation session. Patient achieved 175° active abduction without pain. Discharged to home maintenance program.",
            performedBy: "Dr. Glory Physiotherapist",
            preSessionPain: 1,
            postSessionPain: 0,
    ...
    prescriptions: [
      {
        ...
        status: "Completed",
        targetGoal:
          "Strengthen infraspinatus/teres minor and improve scapular retraction mechanics.",
        generalInstructions:
          "Warm up shoulder with gentle pendulums before starting resistance band work.",
        items: [
          {
            id: "rxi_4",
            exerciseId: "ex_6",
...
export const patientService = {
    ...
    try {
      ...
      if (token && !token.startsWith("mock_")) {
        ...
        const payload = {
          ...
          medicalHistory:
            patientData.medicalHistory?.map((m) => m.condition).join("; ") ||
            "",
        };
  ...
  delete: async (id: string): Promise<boolean> => {
    try {
      ...
      console.warn(
        `[Offline Mode] Archiving/Deleting patient ${id} in localStorage.`,
      );
      const list = getLocalStore();
  ...
  getMedicalHistory: async (patientId: string) => {
    try {
      ...
      console.warn(
        `[Offline Mode] Fetching medical history for patient ${patientId}`,
      );
      const list = getLocalStore();
  ...
  addMedicalHistoryRecord: async (
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Adding medical history record for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient) throw new Error("Patient not found");
      ...
      const newItem = {
        ...
        diagnosedDate: historyData.recordDate
          ? historyData.recordDate.split("T")[0]
          : new Date().toISOString().split("T")[0],
  ...
  getDocuments: async (patientId: string) => {
    try {
      ...
      console.warn(
        `[Offline Mode] Fetching documents for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
  ...
  uploadDocument: async (
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Uploading document for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient) throw new Error("Patient not found");
  ...
  addAssessment: async (
    patientId: string,
    assessmentData: Omit<ClinicalAssessment, "id">,
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Adding clinical assessment for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient) throw new Error("Patient not found");
  ...
  deleteAssessment: async (
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Deleting assessment ${assessmentId} from patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient || !patient.assessments) return false;

      patient.assessments = patient.assessments.filter(
        (a) => a.id !== assessmentId,
      );
      saveLocalStore(list);
      return true;
  ...
  createTreatmentPlan: async (
    patientId: string,
    planData: Omit<TreatmentPlan, "id" | "sessionsCompleted" | "sessions">,
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Creating treatment plan for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient) throw new Error("Patient not found");
  ...
  updatePlanStatus: async (
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Updating treatment plan ${planId} status to ${status}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient || !patient.treatmentPlans)
  ...
  recordTreatmentSession: async (
    patientId: string,
    planId: string,
    sessionData: Omit<TreatmentSession, "id">,
  ): Promise<TreatmentSession> => {
    try {
      const token = localStorage.getItem("gf_auth_token");
      if (token && !token.startsWith("mock_")) {
        const res = await api.post(
          `/patients/${patientId}/treatment-plans/${planId}/sessions`,
          sessionData,
        );
        return res.data;
      }
      ...
      console.warn(
        `[Offline Mode] Recording session on plan ${planId} for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient || !patient.treatmentPlans)
      ...
      plan.sessionsCompleted = Math.min(
        plan.sessionsCount,
        (plan.sessionsCompleted || 0) + 1,
      );
  ...
  createPrescription: async (
    patientId: string,
    prescriptionData: Omit<ExercisePrescription, "id">,
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Creating exercise prescription for patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient) throw new Error("Patient not found");
  ...
  updatePrescriptionStatus: async (
    ...
    try {
      ...
      if (token && !token.startsWith("mock_")) {
        const res = await api.patch(
          `/patients/${patientId}/prescriptions/${prescriptionId}/status`,
          { status },
        );
        return res.data;
      }
      ...
      console.warn(
        `[Offline Mode] Updating prescription ${prescriptionId} status to ${status}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
      if (!patient || !patient.prescriptions)
        throw new Error("Patient or prescriptions not found");
  ...
  deletePrescription: async (
    ...
    try {
      ...
      console.warn(
        `[Offline Mode] Deleting prescription ${prescriptionId} from patient ${patientId}`,
      );
      const list = getLocalStore();
      const patient = list.find((p) => p.id === patientId);
